<main class="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
    ...
    <div class="flex flex-col sm:flex-row items-center gap-3">
        ...
        @if (!$isOwner) 
        <form action="{{ route('colocation.quitter') }}" method="POST" class="w-full sm:w-auto m-0">
                @csrf
                <input type="hidden" name="member_id" value="{{ $membership->id }}">
                <button type="submit" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 bg-white border border-red-100 text-red-500 font-black text-sm rounded-2xl hover:bg-red-50 hover:border-red-200 transition-all shadow-sm group">
                    <i data-lucide="log-out" class="w-4 h-4 text-red-400 group-hover:text-red-500 transition-colors"></i>
                    QUITTER
                </button>
            </form>
        @endif
